...
def save_questionnaire_data(questionnaire_type, score, emotional_score, patient_id):
    try: 
        if questionnaire_type == "PHQ":
            new_entry = PHQ9Scores(score = score, emotional_score = emotional_score, patient_id = patient_id)
            db.session.add(new_entry)
            db.commit()
        elif questionnaire_type == "GAD":
            new_entry = GAD7Scores(score = score, emotional_score = emotional_score, patient_id = patient_id)
            db.session.add(new_entry)
            db.commit()
    except:
...
def save_therapy_data(emotional_state, patient_id, therapist_id, emotion_data):
    try:
        new_sessiondata_entry = SessionData(emotional_state = emotional_state, patient_id = patient_id, therapist_id = therapist_id, emotion_data = emotion_data)
        db.session.add(new_emotiondata_entry)

        i = 0
        for emotion in emotion_data:
            new_emotiondata_entry = EmotionData(emotion_type = emotion, session_id = new_sessiondata_entry.id) 
            db.session.add(new_emotiondata_entry)
            i += 1
        db.session.commit()
    except:
        print("An error has occured while trying to save the therapy session data")
